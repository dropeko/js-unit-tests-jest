...
describe('9 - Implemente os casos de teste da função `getCharacter`', () => {
  ...
  it('Teste se a função retorna o objeto correto para o parâmetro Arya.', () => {
    ...
    expect(getCharacter('Arya')).toMatchObject(aryaSpecs);
  });
  ...
  it('Teste se a função retorna o objeto correto para o parâmetro Brienne', () => {
    ...
    expect(getCharacter('Brienne')).toMatchObject(brienneSpecs);
  });
  
  it('Teste se a função retorna o objeto correto para o parâmetro Melissandre', () => {
    const melissandreSpecs = {
      name: 'Melissandre',
      class: 'Necromancer',
      phrases: ['Death By Fire Is The Purest Death.', 'For The Night Is Dark And Full Of Terrors.'],
    };
    expect(getCharacter('Melissandre')).toMatchObject(melissandreSpecs);
  });
  ...
  it('Teste se ao passar um nome que não está na tabela, a função retorna undefined.', () => {
    expect(getCharacter('Piccolo')).toBeUndefined();
    expect(getCharacter('SpeedRacer')).toBeUndefined();
    expect(getCharacter('Seu Madruga')).toBeUndefined();
  });
});
